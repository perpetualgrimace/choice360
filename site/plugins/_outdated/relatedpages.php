 ...
 *
 * Example: $Options = array('publishedOnly' => false, 'recursionDepth' => 1);
 *
 * This will find all pages, not only published pages, and the depth of recursion
 ...
 * -------------------------------------------------------
 *
 * 'publishedOnly'     (true)     If true, searches only published pages, otherwise all.
 *
 ...
 */
  if (array_key_exists('publishedOnly',$options)) { $options['publishedOnly'] = $options['publishedOnly'];unset($options['publishedOnly']); }
...
/**
 * Get pages from $site->children()->index(), either all or only published pages
 */
  $allPages = site()->children()->index();
  if ($options['publishedOnly']) { $allPages = $allPages->published(); }
